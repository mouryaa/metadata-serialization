...
Can output as RDF (turtle, etc.) or denormalized CSV.
"""
...
Examples:
  ...
  # Generate CSV files
  python generate_linked_data.py -n 10 -y 2 -x 3 -z 5 -o data --format csv
  
  # Generate both RDF and CSV
  python generate_linked_data.py -n 10 -y 2 -x 3 -z 5 -o data --format both
  
  # Use seed for reproducibility
  python generate_linked_data.py -n 50 -y 2 -x 4 -z 10 --seed 42 -o reproducible --format csv
        """
    )
    ...
    parser.add_argument(
        ...
        help="Output format: RDF formats or csv or both (default: turtle)"
    )
    ...
    try:
        ...
        if generate_csv and collector:
            csv_output_file = args.output if args.output.endswith('.csv') else f"{args.output}.csv"
